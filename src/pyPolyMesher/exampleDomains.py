...
def _SuspensionSDF(P):
    ...
    d = dDiff(
        dDiff(dDiff(dDiff(d1, d2), d5), d13),
...
SuspensionDomain = Domain(
    "Suspension Domain", [-2, 24, -2, 24], _SuspensionSDF, _SuspensionBC
)
...
def _MichellBC(Node, BdBox):
    ...
    CircleNodes = [
        i for i, (x, y) in enumerate(Node) if abs((x**2 + y**2) ** 0.5 - 1.0) < eps
    ]
...
def _MbbBC(Node, BdBox):
    ...
    LeftUpperNode = np.where(
        np.logical_and(np.abs(Node[:, 0] - 0) < eps, np.abs(Node[:, 1] - 1) < eps)
    )[0]
    RightBottomNode = np.where(
        np.logical_and(np.abs(Node[:, 0] - 3) < eps, np.abs(Node[:, 1] - 0) < eps)
    )[0]
